...
def encode_prompt(text_encoders, tokenizers, prompt, max_length=[], text_input_ids_list=None, neg_prompt=False, prompt_max_length=None):
    prompt_embeds_list = []
    ...
    for i, text_encoder in enumerate(text_encoders): # create a prompt embeds from each text encoders (there are 2 text encoders)
        ...
        for n in range(0, text_input_ids.shape[-1], max_length[i]):
            

            prompt_embeds = text_encoder(
                text_input_ids[:, n:n+max_length[i]].to(text_encoder.device), output_hidden_states=True, return_dict=False
            )
            '''
            ...
            #print(len(prompt_embeds[1]))
            #print(len(prompt_embeds[2]))
            #print(type(prompt_embeds[0]))
            #print(type(prompt_embeds[1]))
            #print(type(prompt_embeds[2]))
            #print(prompt_embeds[0].shape)
            #print(prompt_embeds[1].shape)
            #print(prompt_embeds[-1][0].shape)
...
def encode_prompt(text_encoders, tokenizers, prompt, text_input_ids_list=None):
    ...
    prompt_embeds = torch.concat(prompt_embeds_list, dim=-1) # combine th
...
def main(args):
    ...
    # import correct text encoder classes (just the classes of the text encoder not the encoder. they will be loaded from the classes below)
    ...
    ****************************************************************************************************
    ****************************************************************************************************
    Load scheduler and different components in models
    ****************************************************************************************************
    ****************************************************************************************************
    '''
    ...
    #unet.config.addition_embed_type = 'text'
    ...
    ****************************************************************************************************
    ****************************************************************************************************
     Create stuff for more efficient training
    ****************************************************************************************************
    ****************************************************************************************************
    '''
    ...
    ****************************************************************************************************
    ****************************************************************************************************
    create and add the save model hook and load model hook to accelerate
    ****************************************************************************************************
    ****************************************************************************************************
    '''
    ...
    ****************************************************************************************************
    ****************************************************************************************************
    set up other things for more ememory efficients training
    ****************************************************************************************************
    ****************************************************************************************************
    '''
    ...
    ****************************************************************************************************
    ...
    if args.train_text_encoder:
        ...
        param_groups.append({"params": te1_params, "lr": args.te1_lr})

        # Text Encoder Two
        te2_params = list(filter(lambda p: p.requires_grad, text_encoder_two.parameters()))
        param_groups.append({"params": te2_params, "lr": args.te2_lr})

    # Now pass param_groups instead of a flat list
    ...
    ****************************************************************************************************
    ...
    # resize the smaller size to 1024 while maintainning the aspect ratio. do this to prevent distoring the image
    train_resize_square = transforms.Resize(args.resolution, interpolation=transforms.InterpolationMode.BICUBIC)
    train_resize_vertical_rec = transforms.Resize((1152, 896), interpolation=transforms.InterpolationMode.BICUBIC)
    train_resize_horizontal_rec = transforms.Resize((896, 1152), interpolation=transforms.InterpolationMode.BICUBIC)                                    
    #train_resize = transforms.Resize((1152, 896), interpolation=transforms.InterpolationMode.BILINEAR)
    ...
    def preprocess_train(examples):
        ...
        for image in images:
            ...
            if args.center_crop:
                ...
                a = 1
                y1, x1, h, w = train_crop.get_params(image, (args.resolution, args.resolution)) # /////////// get the parameter for cropping when it is random crop
    ...
    def preprocess_train(examples):
        ...
        crop_top_lefts = []
        for image in images:
            original_sizes.append((image.height, image.width)) # this is the original one
            
            #original_sizes.append((0, 0))
            '''if image.height == image.width:
                image = train_resize_square(image)
            elif image.height > image.width:
                image = train_resize_vertical_rec(image)
            else:
                image = train_resize_horizontal_rec(image)'''
            image = train_resize_vertical_rec(image)
            #image = train_resize(image) # ////////////////////////////////////////////////////////////
            ...
            if args.center_crop:
                ...
                a = 1
                #y1, x1, h, w = train_crop.get_params(image, (args.resolution, args.resolution)) # /////////// get the parameter for cropping when it is random crop
                '''if (image.height - args.resolution) > args.resolution:
                    y1 = math.ceil(0.1*(image.height- args.resolution))
                else:
                    y1 = 0
                if (image.width - args.resolution) > args.resolution:
                    x1 = math.ceil(0.2*(image.height- args.resolution))
    ...
    with accelerator.main_process_first():
        ...
        # The with_transform function in the Hugging Face datasets library is used to apply a transformation function (like preprocess_train) to the dataset when it's loaded or accessed.

        train_dataset = dataset["train"].with_transform(preprocess_train, output_all_columns=True) # the preprocess_train function tokenize and preprocess the image
    

    '''
    ****************************************************************************************************
    ****************************************************************************************************
    the collate_fn function to be passed  to Pytorch DataLoader
    ****************************************************************************************************
    ****************************************************************************************************
    '''

    # the function in with_transform function(preprocess_train in this case) is apply to the examples passed to the custom collate_fn function
    # and pass the return value to the examples parameter of the collate_fn function
    ...
    ****************************************************************************************************
    ****************************************************************************************************
    create the Dataloader, learning rate scheduler
    ****************************************************************************************************
    ****************************************************************************************************
    '''
    ...
    ****************************************************************************************************
    ****************************************************************************************************
    prepare everything with the accelerator before training
    ****************************************************************************************************
    ****************************************************************************************************
    '''
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                ****************************************************************************************************
                ****************************************************************************************************
                create additional time related information to be passed alongside with the pooled_text_embeddings
                this is sued to condition the generation process of the image with condition at each time step
                useful for image that have temporal chracteristic like videos
                ****************************************************************************************************
                ****************************************************************************************************
                '''
                # time ids
                def compute_time_ids(original_size, crops_coords_top_left):
                    ...
                    #target_size = (args.resolution, args.resolution) # this is the original one
                ...
                ****************************************************************************************************
                ...
                prompt_embeds, pooled_prompt_embeds = encode_prompt(
                    ...
                    tokenizers=None,
                    prompt=None,
                    max_length=[tokenizer_one.model_max_length, tokenizer_two.model_max_length],
                ...
                ****************************************************************************************************
                ...
                if args.snr_gamma is None:
                    loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean") # /////// this will run if do normally
                else:
                ...
                total_mean += avg_loss.item() / args.gradient_accumulation_steps
                ...
                ****************************************************************************************************
                ...
                '''total_norm = 0.0
                for param in unet.parameters():  # or use UNet/TextEncoder if you're training parts
                ...
                if accelerator.sync_gradients:
                    ...
                    for group in param_groups:
                        params_to_optimize.extend(group["params"])
                    accelerator.clip_grad_norm_(params_to_optimize, args.max_grad_norm) # this is to prevent exploding gradient by ensuring that the graidents don't
                                                                                        # exceed the maximum args.max_grad_norm gradient value
                optimizer.step()
                lr_scheduler.step()
        ...
        ****************************************************************************************************
        ****************************************************************************************************
        do validation once finished training if pass the validation prompt
        ****************************************************************************************************
        ****************************************************************************************************
        '''
        if accelerator.is_main_process:
    ...
    ****************************************************************************************************
    ...
    if accelerator.is_main_process:
        ...
        ****************************************************************************************************
        ****************************************************************************************************
        do inference if pass the inference prompt
        ****************************************************************************************************
        ****************************************************************************************************
        '''
        # Final inference
        ...
        ****************************************************************************************************
        ****************************************************************************************************
        push the model to model hub if specify
        ****************************************************************************************************
        ****************************************************************************************************
        '''
        if args.push_to_hub:
